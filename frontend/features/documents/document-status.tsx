...
export function DocumentStatus({ documentId }: Readonly<{ documentId: string }>) {
  ...
  return <section className="max-w-2xl"><p className="text-sm font-medium text-accent">Documents · F2</p><h1 className="mt-2 text-3xl font-semibold tracking-tight">Ingestion status</h1><div className="mt-6 rounded-lg border border-line bg-panel p-5 shadow-panel"><dl className="space-y-4"><div><dt className="text-sm text-muted">Filename</dt><dd className="mt-1 font-medium">{document.filename}</dd></div><div><dt className="text-sm text-muted">State</dt><dd className="mt-1 font-medium capitalize">{document.state}</dd></div><div><dt className="text-sm text-muted">Job status</dt><dd className="mt-1 font-medium capitalize">{document.ingestion_status}</dd></div></dl>{!terminalStates.has(document.state) && <p className="mt-5 text-sm text-muted" role="status">This page checks the individual document status every two seconds.</p>}</div></section>;
}
